...
import { WorkerEventsGateway } from '../websocket.gateway.js';
...
describe('WorkerEventsGateway (integration-light)', () => {
  ...
  test('throttle des écritures DB et émission immédiate vers la room', async () => {
    ...
    expect(emitted.filter(e => e.event === 'progress' && e.room === `job:${jobId}`)).toHaveLength(2);
    ...
    expect(downloads.updateJobProgress).toHaveBeenCalledWith(jobId, 55, 'download', undefined, undefined, undefined);
  });
  ...
  test('flush sur completed: clear timers, update DB, émettre completed', async () => {
    ...
    await gw.handleCompleted({ jobId, filename: 'foo.mp4', size: 1234, outputPath: '/tmp/foo.mp4' });

    // setJobCompleted appelé
    expect(downloads.setJobCompleted).toHaveBeenCalledTimes(1);

    // event completed émis
    const completedEvents = emitted.filter(e => e.event === 'completed' && e.room === `job:${jobId}`);
  ...
  test('failed: clear timers, update status, émettre failed', async () => {
    ...
    await gw.handleFailed({ jobId, errorCode: 'NETWORK_ERROR', message: 'timeout' });

    expect(downloads.updateJobStatus).toHaveBeenCalledWith(jobId, 'failed', 'NETWORK_ERROR', 'timeout');
    const failedEvents = emitted.filter(e => e.event === 'failed' && e.room === `job:${jobId}`);
